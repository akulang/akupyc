...
class Parser:
    def __init__(self, lexer, emitter, usestdlib, libraries):
        self.lexer = lexer
    ...
    def isComparisonOperator(self):
        return self.checkToken(TokenType.GT) or self.checkToken(TokenType.GTEQ) or self.checkToken(TokenType.LT) or self.checkToken(TokenType.LTEQ) or self.checkToken(TokenType.EQEQ) or self.checkToken(TokenType.BANGEQ) or self.checkToken(TokenType.LOR) or self.checkToken(TokenType.LAND)
    ...
    def shift(self, porfunc, scope):
        self.term(porfunc, scope)

        while self.checkToken(TokenType.RSHIFT) or self.checkToken(TokenType.LSHIFT):
            porfunc(self.curToken.text)
            self.nextToken()
            self.term(porfunc, scope)
    
    def binary(self, porfunc, scope):
        self.shift(porfunc, scope)

        while self.checkToken(TokenType.BAND) or self.checkToken(TokenType.BOR):
            porfunc(self.curToken.text)
            self.nextToken()
            self.shift(porfunc, scope)
    ...
    def primary(self, porfunc, scope):
        ...
        if self.checkToken(TokenType.NUMBER):
            ...
            elif self.checkPeek(TokenType.LSQUARE):
                self.nextToken()
                self.match(TokenType.LSQUARE)
                if not ename in scope.keys():
                    if not ename in self.globals.keys():
                        self.abort("referencing a symbol that isn't assigned yet or doesn't exist `%s`" % ename)
                porfunc("%s[" % ename)
                self.expression(porfunc, scope)
                self.match(TokenType.RSQUARE)
                porfunc("]")
    ...
    def program(self):
        ...
        if self.usestdlib:
            self.emitter.headerLine(akulib)
        else:
            self.emitter.headerLine("// --nostdlib option was passed")
        if self.libraries.get("metal"):
            self.emitter.headerLine(metallib)
        elif self.libraries.get("std"):
            if not self.usestdlib:
                self.emitter.headerLine(akulib)
            else:
                self.abort("cannot use package std if --nostdlib is not passed")
